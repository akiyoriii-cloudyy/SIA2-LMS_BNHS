...
class GateRecordsViewModel(
    ...
    var logs by mutableStateOf<List<GateLogRecord>>(emptyList())
    ...
    var editing by mutableStateOf(GateLogInput())
    ...
    fun refresh() {
        viewModelScope.launch {
            ...
            filter = filter.copy(date = fmt.format(filterDate))
            logs = repository.listGateLogs(filter)
            isLoading = false
            statusMessage = "${logs.size} gate record(s)."
    ...
    fun openCreate() {
        editingId = 0L
        editing = GateLogInput(date = filterDate)
        formError = null
        showEditor = true
    }

    fun openEdit(record: GateLogRecord) {
        editingId = record.id
        editing = GateLogInput(
    ...
    fun save() {
        viewModelScope.launch {
            ...
            val result = if (editingId > 0L) {
                repository.updateGateLog(editing, editingId, actorEmail)
            } else {
                repository.createGateLog(editing, actorEmail)
            }
            isLoading = false
            when (result) {
                is RecordsResult.Success -> {
                    showEditor = false
                    formError = null
                    statusMessage = if (editingId > 0L) "Gate record updated." else "Gate record created."
                    refresh()
                }
    ...
    fun delete(record: GateLogRecord) {
        viewModelScope.launch {
            when (val result = repository.deleteGateLog(record.id, actorEmail)) {
                is RecordsResult.Success -> {
                    statusMessage = "Gate record deleted."
                    refresh()
                }
    ...
    class Factory(private val repo: AttendanceRecordsRepository, private val email: String) : ViewModelProvider.Factory {
        ...
        override fun <T : ViewModel> create(c: Class<T>): T = GateRecordsViewModel(repo, email) as T
    }
}

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun GateRecordsScreen(
    ...
    var toDelete by remember { mutableStateOf<GateLogRecord?>(null) }
    val canWrite = rbac.canManageAttendance()

    Column(modifier.fillMaxSize()) {
        Text("Gate Access Records", fontWeight = FontWeight.Black, color = PrimaryDark, fontSize = 16.sp)
        Text("Security CRUD — create, read, update, delete gate logs", fontSize = 12.sp, color = TextSubtitle)
        Spacer(Modifier.height(8.dp))
        OutlinedTextField(
            value = viewModel.filter.query,
            onValueChange = { viewModel.filter = viewModel.filter.copy(query = it); viewModel.refresh() },
            modifier = Modifier.fillMaxWidth(),
            placeholder = { Text("Search student, LRN, RFID…") },
            leadingIcon = { Icon(Icons.Default.Search, null) },
            singleLine = true,
        )
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            AttendanceRecordsRepository.GATE_STATUSES.forEach { st ->
                FilterChip(
            ...
            items(viewModel.logs, key = { it.id }) { log ->
                Card(colors = CardDefaults.cardColors(containerColor = Color.White)) {
                    Column(Modifier.padding(12.dp)) {
                        ...
                        Text("${log.date} · ${log.status} · ${log.rfidUid}", fontSize = 11.sp, color = TextSubtitle)
                        if (canWrite) {
                            Row {
    ...
    if (viewModel.showEditor) {
        AlertDialog(
            ...
            title = { Text(if (viewModel.editingId > 0L) "Edit gate log" else "New gate log") },
            text = {
                        ...
                        ExposedDropdownMenu(expanded = expanded, onDismissRequest = { expanded = false }) {
                            viewModel.students.forEach { s ->
                                DropdownMenuItem(
                                    text = { Text(s.name) },
                                    onClick = {
                                        viewModel.editing = viewModel.editing.copy(studentId = s.id.toLong())
                                        expanded = false
                                    },
                                )
                            }
...
@Composable
fun rememberGateRecordsViewModel(repo: AttendanceRecordsRepository, email: String): GateRecordsViewModel {
    return androidx.lifecycle.viewmodel.compose.viewModel(factory = GateRecordsViewModel.Factory(repo, email))
}
