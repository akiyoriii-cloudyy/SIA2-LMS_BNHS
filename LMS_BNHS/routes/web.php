...
Route::middleware(['auth'])->group(function (): void {
    ...
    Route::middleware(['role:admin,adviser,subject_teacher', 'permission:activity_logs.view'])->prefix('admin')->name('admin.')->group(function (): void {
        ...
        Route::middleware(['role:admin', 'permission:security_audit.view'])->group(function (): void {
            Route::get('/security-audit', [AdminSecurityAuditController::class, 'index'])->name('security-audit.index');
            Route::get('/security-audit/alerts', [AdminSecurityAuditController::class, 'alerts'])->name('security-audit.alerts');
            Route::get('/security-audit/reports', [AdminSecurityAuditController::class, 'reports'])->name('security-audit.reports');
            Route::get('/security-audit/export', [AdminSecurityAuditController::class, 'export'])->name('security-audit.export');
            Route::get('/security-audit/{securityAuditLog}', [AdminSecurityAuditController::class, 'show'])->name('security-audit.show');
            Route::post('/security-audit/{securityAuditLog}/resolve', [AdminSecurityAuditController::class, 'resolve'])->name('security-audit.resolve');
            Route::post('/security-audit/alerts/{securityAlert}/acknowledge', [AdminSecurityAuditController::class, 'acknowledge'])->name('security-audit.alerts.acknowledge');
            Route::post('/security-audit/alerts/{securityAlert}/deactivate', [AdminSecurityAuditController::class, 'deactivate'])->name('security-audit.alerts.deactivate');
        });

        // Transaction Management Routes
        Route::middleware(['role:admin', 'permission:transactions.view'])->group(function (): void {
            Route::get('/transactions', [AdminTransactionController::class, 'index'])->name('transactions.index');
            Route::get('/transactions/logs', [AdminTransactionController::class, 'logs'])->name('transactions.logs');
            ...
            Route::post('/transactions/{transaction}/rollback', [AdminTransactionController::class, 'rollback'])->name('transactions.rollback');
        });
    });
